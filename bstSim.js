...
//Draw left/right indicator
function drawIndicator(ctx, distX, distY, radius, data) {
   ctx.font = "10px Arial";
   ctx.fillText(data, distX, distY - radius + 10);
}
...
function nodeInsert(root, value, ctx, index) {
   if (root === null) {
      ...
      distX !== false &&
         drawIndicator(ctx, root.right.x, root.right.y, root.right.radius, "R");
      ...
      distX !== false &&
         drawIndicator(ctx, root.left.x, root.left.y, root.left.radius, "L");
